...
import { Icon } from '../icons/Icon'; // Caminho de importação corrigido
  ...
  return (
    <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[100] w-full max-w-md flex flex-col items-center space-y-2">
      {toasts.map((toast) => (
        <div
          key={toast.id}
          className={`w-full bg-light-card dark:bg-dark-card border border-light-border dark:border-dark-border rounded-lg shadow-lg flex items-start p-4 animate-fade-in-down border-l-4 ${getBorderColorClass(toast.type)}`}
        >
