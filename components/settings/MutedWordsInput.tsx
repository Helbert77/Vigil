...
const MutedWordsInput: React.FC<MutedWordsInputProps> = ({ mutedWords, onAddWord, onRemoveWord }) => {
  ...
  return (
    <div>
      <p className="font-medium text-gray-800 dark:text-gray-200">{t('privacy.mutedWordsTitle')}</p>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">{t('privacy.mutedWordsDescription')}</p>
      <div className="flex items-center space-x-2 mb-3">
        <input
          ...
          placeholder={t('privacy.addNewWord')}
      ...
      <div className="flex flex-wrap gap-2">
        {mutedWords.map((word: string) => (
          <div key={word} className="flex items-center bg-gray-200 dark:bg-gray-700 rounded-full px-3 py-1 text-sm">
            <span>{word}</span>
            <button onClick={() => onRemoveWord(word)} className="ml-2 text-gray-600 dark:text-gray-300 hover:text-red-500" aria-label={`Remove ${word}`}>
              <XIcon />
            </button>
          </div>
        ))}
