...
interface CreateAdCommentProps {
  user: User;
  adId: string;
  onAddComment: (commentText: string, imageUrl?: string) => void;
  allUsers: User[];
  placeholder?: string;
}

const CreateAdComment: React.FC<CreateAdCommentProps> = ({ user, adId, onAddComment, allUsers, placeholder = "Adicione um comentário..." }) => {
  const { t } = useTranslation(['posts', 'ads', 'common']);
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filePath = `${user.id}/ad-comments/${Date.now()}.${fileExt}`;
        ...
        <form onSubmit={handleSubmit}>
          ...
          {imageUrl && (
            <div className="mt-2 relative w-32 h-32">
              ...
              <button type="button" onClick={() => setImageUrl(undefined)} className="absolute top-1 right-1 bg-black/50 text-white rounded-full p-1"><XIcon /></button>
            </div>
          )}
...
export default React.memo(CreateAdComment);