...
const AddEventImageModal: React.FC<AddEventImageModalProps> = ({ event, onClose, onImageAdded }) => {
  ...
  const handleSubmit = async () => {
    if (!imageUrl) {
      addToast('Por favor, insira uma URL ou faça upload de uma imagem.', 'error');
      return;
    }
    try {
      ...
      addToast('Falha ao atualizar a imagem do evento.', 'error');
    }
  };
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-2 md:p-6 animate-scale-in">
      <div className="bg-light-card dark:bg-dark-card rounded-lg md:rounded-2xl max-w-lg w-full max-h-[95vh] md:max-h-[80vh] flex flex-col border border-light-border dark:border-dark-border shadow-2xl overflow-hidden">
        <div className="p-3 md:p-6 flex items-center justify-between border-b border-light-border dark:border-dark-border">
          <h2 className="text-lg md:text-xl font-bold text-gray-800 dark:text-white truncate pr-2">Adicionar Imagem ao Evento</h2>
        ...
        <div className="p-3 md:p-6 space-y-3 md:space-y-4 overflow-y-auto">
          <p className="text-sm text-gray-500 dark:text-gray-400">Para o evento: <span className="font-semibold">{event.title}</span></p>
          <div>
        ...
        <div className="p-3 md:p-6 flex justify-end gap-3 border-t border-light-border dark:border-dark-border">
          <button onClick={onClose} className="px-6 py-3 md:py-2 rounded-lg border border-light-border dark:border-dark-border hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-base">Cancelar</button>
          <button onClick={handleSubmit} className="px-6 py-3 md:py-2 rounded-lg bg-cyan-600 hover:bg-cyan-700 text-white font-medium transition-colors text-base">Salvar Imagem</button>
        </div>
      </div>
    </div>
  );
