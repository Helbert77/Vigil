...
const Profile: React.FC<ProfileProps> = ({ user, posts, onUpdatePost, savedPostIds, onToggleSave, onUpdateUser }) => {
  ...
  useEffect(() => {
    // When editing starts, populate the form with current user data
    if (isEditing) {
      setEditedUser({
        name: user.name,
  ...
  useEffect(() => {
    // Reset editing mode when the user prop changes, e.g., navigating from one profile to another
    setIsEditing(false);
  }, [user.id]);
  ...
  return (
    <div>
      ...
      <h3 className="text-xl font-bold mb-4">{onUpdateUser ? 'Your Posts' : 'Posts'}</h3>
      <div>
        {userPosts.length > 0 ? (
          userPosts.map((post) => 
            <PostCard 
