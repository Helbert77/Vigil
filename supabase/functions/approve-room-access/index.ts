...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (requestError || !accessRequest) {
      console.error('[approve-room-access] Error fetching access request:', requestError);
      return new Response(
        JSON.stringify({ 
    ...
    if (roomError || !room) {
      console.error('[approve-room-access] Error fetching room:', roomError);
      return new Response(
        JSON.stringify({ 
    ...
    if (room.created_by !== user.id) {
      return new Response(
        JSON.stringify({ error: 'Apenas o criador da sala pode aprovar pedidos', success: false }), 
    ...
    if (updateError) {
      console.error('[approve-room-access] Error updating request:', updateError);
      return new Response(
        JSON.stringify({ 
          error: `Erro ao aprovar pedido: ${updateError.message}`, 
          success: false 
        }), 
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    // 8. Create invitation in chat_room_invitations
    const { error: invitationError } = await supabaseAdmin
      .from('chat_room_invitations')
      .insert({
        room_id: accessRequest.room_id,
        inviter_id: user.id,
        invitee_id: accessRequest.requester_id,
        status: 'pending'
      });

    if (invitationError) {
      console.error('[approve-room-access] Error creating invitation:', invitationError);
      ...
      return new Response(
        JSON.stringify({ 
          error: `Erro ao criar convite: ${invitationError.message}`, 
          success: false 
        }), 
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    // 9. Create notification for requester
    ...
    if (notificationError) {
      console.error('[approve-room-access] Error creating notification:', notificationError);
    ...
    const errorMessage = error instanceof Error ? error.message : 'Erro desconhecido ao aprovar pedido';
    console.error('[approve-room-access] Unexpected error:', error);
